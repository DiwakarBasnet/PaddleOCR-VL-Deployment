...
def process_pdf(pdf_path, output_md_path=None, prompt="ocr", dpi=300):
    ...
    full_markdown.append(f"*Processed on {time.strftime('%Y-%m-%d %H:%M:%S')} with DPI {dpi}*")
    ...
    for page_num in range(num_pages):
        ...
        # 300 DPI is usually standard for high quality OCR
        ...
        try:
            ...
            with urllib.request.urlopen(req) as response:
                ...
                if result.get("success"):
                    print(f"  Page {page_num + 1} Success! (Latency: {latency:.2f}ms)")
                    ...
                    print(f"  Page {page_num + 1} Failed: {result.get('error')}")
    ...
    if output_md_path:
        ...
        out_name = os.path.splitext(os.path.basename(pdf_path))[0] + "_ocr_result.md"
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Process a PDF file with PaddleOCR-VL API and get markdown output.")
    parser.add_argument("pdf_path", help="Path to the PDF file")
    parser.add_argument("--output", "-o", help="Optional output markdown file path")
    parser.add_argument("--prompt", "-p", default="ocr", help="OCR prompt (default: 'ocr')")
    parser.add_argument("--dpi", "-d", type=int, default=300, help="DPI for PDF conversion (default: 300)")
