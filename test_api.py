    ...
    try:
        async with session.post(url, json=payload, timeout=600) as response:
            ...
            if response.status == 200:
                ...
                print(f"Request {req_id}: Failed ({response.status}) in {latency:.2f}ms")
                return {
                    "id": req_id,
                    "success": False,
                    "status": response.status,
...
def save_results_to_markdown(results, output_file):
    ...
    with open(output_file, 'w') as f:
        ...
        f.write(f"**Success Rate:** {success_count}/{len(results)} ({products_pct(success_count, len(results))})\n")
        f.write(f"**Average Latency:** {avg_latency:.2f} ms\n")
        f.write(f"**Min/Max Latency:** {min_latency:.2f} ms / {max_latency:.2f} ms\n\n")
        ...
        for r in results:
            ...
            f.write(f"| {r['id']} | {r['status']} | {r['latency']:.2f} | {'✅' if r['success'] else '❌'} | {error_msg} |\n")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--output", type=str, default="api_test_results.md", help="Output Markdown file")
    
    args = parser.parse_args()
    
    try:
        results = asyncio.run(run_load_test(args.url, args.requests, args.concurrency, args.image_dir))
