...
from main import app, paddle_image as image, PaddleOCRService
...
@modal.asgi_app()
def fastapi_app():
    """FastAPI web application for PaddleOCR-VL (Updated)"""

    web_app = FastAPI(
        title="PaddleOCR VL API (Updated)",
    ...
    async def root():
        return {
            "service": "PaddleOCR VL API (Updated)",
            "version": "3.3.0",
            "status": "running",
